...
The 'syncing' status indicator on the coin shows that the dEiRA app tries to synchronize itself with a given blockchain.
...
Keep in mind, dEiRA app keeps the synchronization process running only when the dEiRA app remains open. If the app is closed the synchronization process is paused and only resumes when the app is opened again.
